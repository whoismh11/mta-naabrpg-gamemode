...
addEventHandler ( "onResourceStart", resourceRoot, function ( )
	...
	for i, v in pairs ( bankLocations ) do
		...
		createBlip ( bx, by, bz, 52, 2, 255, 255, 255, 255, 0, 450 )
...
addEventHandler ( "NGBank:ModifyAccount", root, function ( action, amount )
	...
	if ( isGuestAccount ( acc ) ) then
		return exports['NGMessages']:sendClientMessage ( "Baraye estefade az system Bank lotfan login konid.", source, 200, 200, 200 )
	end
	...
	if ( action == 'withdraw' ) then
		if ( bankBalance < amount ) then
			return exports['NGMessages']:sendClientMessage ( "Shoma in mablagh ra dar hesabe bankie khod nadarid.", source, 200, 200, 200 )
		end
		accounts[acc] = accounts[acc] - amount
		givePlayerMoney ( source, amount )
		exports['NGMessages']:sendClientMessage ( "Shoma $"..tostring ( amount ).." az hesabe bankie khod bardasht kardid.", source, 200, 200, 200 )
		exports['NGLogs']:outputActionLog ( getPlayerName ( source ).." $"..tostring(amount).." be hesabe bankie khod variz kard. (Majmoo: $"..accounts[acc]..")" )
		lg = getPlayerName ( source ).." bardasht $"..amount
	elseif ( action == 'deposit' ) then
		...
		exports['NGMessages']:sendClientMessage ( "Shoma $"..tostring ( amount ).." be hesabe bankie khod variz kardid.", source, 200, 200, 200 )
		exports['NGLogs']:outputActionLog ( getPlayerName ( source ).." $"..tostring(amount).." be hesabe bankie khod variz kard. (Majmoo: $"..accounts[acc]..")" )
		lg = getPlayerName ( source ).." variz $"..amount
	end
