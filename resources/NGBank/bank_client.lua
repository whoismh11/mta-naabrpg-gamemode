...
bank['window'] = guiCreateWindow( ( sx / 2 - 446 / 2 ), ( sy / 2 - 363 / 2 ), 446, 363, "Hesabe Banki - N/A", false)
...
bank.label[2] = guiCreateLabel(10, 84, 213, 20, "Esme Hesab: N/A", false, bank['window'])
bank.label[3] = guiCreateLabel(10, 114, 213, 20, "Mojodie Hesab: $0", false, bank['window'])
bank.label[4] = guiCreateLabel(0, 159, 446, 49, string.rep ( "_", 85 ).."\nEghdamate Karbar\n"..string.rep ( "_", 85 ), false, bank['window'])
bank.label[5] = guiCreateLabel(10, 218, 81, 19, "Mablagh:", false, bank['window'])
bank.label[6] = guiCreateLabel(0, 3, 203, 18, "Pardazeshe Darkhast...", false, bank['progress'])
bank.radio['deposit'] = guiCreateRadioButton(9, 272, 101, 20, "Variz", false, bank['window'])
bank.radio['withdraw'] = guiCreateRadioButton(110, 272, 113, 20, "Bardasht", false, bank['window'])
bank.button['go'] = guiCreateButton(233, 272, 203, 30, "Edameye Darkhast", false, bank['window'])
bank.button['exit'] = guiCreateButton(233, 312, 203, 30, "Khoroj Az Bank", false, bank['window'])
...
addEventHandler ( "NGBank:onClientEnterBank", root, function ( money, account, marker )
	...
	guiSetText ( bank['window'], "Hesabe Banki - ".. tostring ( account ) )
	guiSetText ( bank['amount'], "" )
	guiSetText ( bank.label[2], "Esme Hesab: "..tostring ( account ) )
	guiSetText ( bank.label[3], "Mojodie Hesab: $"..convertNumber ( money ) )
	guiSetText ( bank.label[6], "Pardazesh - 0%" )
...
function CloseBankWindow ( )
	...
	guiSetVisible ( bank['progress'], false )
	guiSetText ( bank['amount'], "" )
	guiSetText ( bank.label[2], "Esme Hesab: N/A" )
	guiSetText ( bank.label[3], "Mojodie Hesab: $0" )
	guiSetText ( bank.label[6], "Pardazesh - 0%" )
	guiProgressBarSetProgress ( bank['progress'], 0 )
	...
	elseif ( source == bank.button['go'] ) then
		if ( guiGetText ( bank['amount'] ) ~= "" ) then
			...
			progressTimer = setTimer ( function ( )
				...
				guiSetText ( bank.label[6], "Pardazesh - "..tostring ( guiProgressBarGetProgress ( bank['progress'] ) ).."%" )
				if ( guiProgressBarGetProgress ( bank['progress'] ) >= 100 and guiGetVisible ( bank['window'] ) ) then
					...
					setAllEnabled ( true )
					guiSetText ( bank.label[6], "Darkhast Anjam Shod" )
			...
			exports['NGMessages']:sendClientMessage ( "Lotfan mablagh ra vared konid", 200, 200, 200 )
		end
	end
end
...
addEventHandler ( "NGBanks:resfreshBankData", root, function ( amount )
	guiSetText ( bank.label[3], "Mojodie Hesab: $"..convertNumber ( amount ) )
end )
