...
addCommandHandler ( "vipchat", function ( p, cmd, ... )
	if ( not isPlayerVIP ( p ) ) then
		return exports.NGMessages:sendClientMessage ( "In dastoor faghat baraye afrade VIP mibashad", p, 255, 0, 0 )
	end
	
	if ( isPlayerMuted ( p ) ) then
		return exports.NGMessages:sendClientMessage ( "In dastoor vaghti ke shoma mute bashid gheyre faal mishavad", p, 255, 255, 0 )
	end
	
	if ( not getElementData ( p, "userSettings" )['usersetting_display_vipchat'] ) then
		return exports.NGMessages:sendClientMessage ( "Baraye estefade az in dastoor lotfan aval VIP Chat ra az mobile khod faal konid.", 0, 255, 255 )
	end
...
function checkPlayerVipTime ( p )
	...
	if ( isDatePassed ( expDate ) ) then
		...
		exports.NGMessages:sendClientMessage ( "Zamane VIP boodane shoma be etmam reside.", p, 255, 0, 0 )
	end
end
...
VipPayoutTimer = setTimer ( function ( )
	...
	for i, v in ipairs ( getElementsByType ( "player" ) ) do
		if ( isPlayerVIP ( v ) ) then
			...
			exports.NGMessages:sendClientMessage ( "Chonke VIP hastid, in $"..money.." ra rayegan daryaft kardid!", v, 0, 255, 0 )
		end
	end
end, (60*60)*1000, 0 ) 
