...
function EventCore.StartEvent ( id )
	...
	local text = textCreateTextItem ( "Shoroe Event Dar: 30", 0.6, 0.65, "hight", 255, 255, 0, 255, 2, "right", "bottom", 2 ) 
	...
	DummyTimer1 = setTimer ( function ( )
		...
		textItemSetText ( eventInfo.dispText, "Shoroe Event Dar: "..math.floor(getTimerDetails(eventInfo.timer)/1000) )
	end, 1000, 0 )
	...
	exports.NGMessages:sendClientMessage ( "Event: "..tostring(events[id].name).." sakhte shod! Shoro 30 saniye dige, az /joinevent baraye join shodan estefade konid!", root, 0, 255, 0 )
...
function EventCore.PlayerJoinEventCommand ( p )
	if ( playersInEvent [ p ] ) then
		return exports.NGMessages:sendClientMessage ( "WTF? Shoma dar event hastid!", p, 0, 255, 255 )
	end
	
	local rSlots = eventInfo.maxSlots-table.len(playersInEvent)
	if ( getElementData ( p, "NGPolice:JailTime" ) ) then return exports.NGMessages:sendClientMessage ( "Darhali ke dar zendan hastid nemitavanid be event beravid!", p, 255, 0, 0 ) end
	if ( isPedInVehicle ( p ) ) then return exports.NGMessages:sendClientMessage ( "Lotfan ebteda az vasile naghlie kharej shavid", p, 255, 255, 0 ) end
	if ( getPlayerWantedLevel ( p ) > 0 and EventCore.GetDistanceToNearestCop ( p ) <= 170 ) then return exports.NGMessages:sendClientMessage ( "Shoma tahte taghib hastid va yek afsare police nazdik shoma vojod darad. Shoma nemitavanid be event beravid.", p, 255, 255, 0 ) end
	if ( getElementInterior ( p ) ~= 0 ) then return exports.NGMessages:sendClientMessage ( "Baraye raftan be event lotfan ebteda az in makan kharej shavid.", p, 255, 255, 0 ) end
	if ( rSlots <= 0 ) then return exports.NGMessages:sendClientMessage ( "Dir kardi.. Event por shod!", p, 255, 0, 0 ) end
	...
	if ( rSlots > 0 ) then exports.NGMessages:sendClientMessage ( "Event: "..getPlayerName(p).." be event molhagh shod. Faghat "..rSlots.." slote dige moonde...", root, 255, 255, 2550 )
	else exports.NGMessages:sendClientMessage ( "Event: "..getPlayerName(p).." be event molhagh shod. Zarfiate event por shod.", root, 255, 0, 0 ) end
...
function EventCore.BeginTheEvent ( )
	...
	if ( #EventCore.GetPlayersInEvent ( ) < eventInfo.minSlots ) then
		EventCore.EndEvent ( )
		return exports.NGMessages:sendClientMessage ( "In event be dalile kafi nabodane tedad laghv shod!", root, 255, 255, 0 )
	end
	
	exports.NGMessages:sendClientMessage ( "Event: Ejaze dahid bazi shoro shavad!", root, 0, 255, 0 )
	...
	setTimer ( function ( ) 
		for i, v in pairs ( EventCore.GetPlayersInEvent ( ) ) do
			if ( getElementAlpha ( v ) == 200  ) then
				exports.NGMessages:sendClientMessage ( "In event dar khod bug shenasayi kard! Event darhale laghv shodan mibashad.", root, 255, 0, 0 )
				EventCore.EndEvent ( )
				break
			end
		end
	...
	if ( eventInfo ) then
		for i, v in pairs ( playersInEvent ) do
			if ( isElement ( i ) ) then
				...
				exports.NGMessages:sendClientMessage("Moteasefim babate mordane gheyre montazere. Resource event darhale ejraye event motevaghef shod.", i, 255, 0, 0 )
			end
		end
	end
end )
...
function EventCore.RemovePlayerByWasted ( )
	...
	exports.NGMessages:sendClientMessage ( getPlayerName ( source ).." Dar event mord!", root, 255, 255, 0 )
end
...
function EventCore.WinPlayerEvent ( p )
	if ( isElement ( p ) ) then
		...
		exports.NGMessages:sendClientMessage ( "Event: "..getPlayerName(p).." $"..a.." baabate bordane evente "..eventInfo.name.." daryaft kard!", root, 0, 255, 0 )
...
addCommandHandler ( "makeevent", function ( p, cmd, id )
	if ( exports.ngadministration:getPlayerStaffLevel ( p, 'int' ) >= 2 ) then
		...
		if ( not events [ id ] ) then
			return exports.NGMessages:sendClientMessage ( "ID event eshtebah ast. range dorost az 1 ta "..table.len ( events ).." mibashad.", p, 255, 255, 0 )
		end
		if ( eventInfo ) then
			return exports.NGMessages:sendClientMessage ( "Dar hale hazer event dar hale ejra ast", p, 255, 255, 0 )
		end
...
addCommandHandler ( "stopevent",  function ( p )
	...
	if ( not eventInfo ) then
		return exports.NGMessages:sendClientMessage ( "Dar in lahze hich eventi dar hale ejra nist.", p, 255, 255, 0 )
	end
	--exports.NGLogs:outputActionLog ( getPlayerName ( p ).." has stopped the event (event: "..eventInfo.name..")" )
	exports.NGMessages:sendClientMessage ( "Event tavasote admin level "..tostring(l).." "..getPlayerName(p).." laghv shod.", root, 255, 0, 0 )
	EventCore.EndEvent ( )
end )
...
addCommandHandler ( "leaveevent", function ( p )
	if ( isPlayerInEvent ( p ) ) then
		if ( not eventInfo.allowLeaveCommand ) then
			return exports.NGMessages:sendClientMessage ( "Shoma mojaz be tarke in event nistid.", p, 255, 0, 0 )
		end
		
		EventCore.RemovePlayerFromEvent ( p, true )
		exports.NGMessages:sendClientMessage ( "Event: "..getPlayerName(p).." event ra tark kard", root, 255, 255, 0 )
		--exports.NGLogs:outputActionLog ( getPlayerName ( p ).." has left the "..eventInfo.name.." event" )
	else
		exports.NGMessages:sendClientMessage ( "Shoma dar event nistid", p, 255, 255, 255 )
	end
end )

addCommandHandler ( "eventhelp", function ( p )
	outputChatBox ( "----- Event Help -----", p, 0,255, 255 )
	outputChatBox ( "/joinevent - Vorod be event dar hale ejra", p, 0, 255, 255 )
	outputChatBox ( "/leaveevent - Koshtane khod va kharej shodan az event", p, 0, 255, 255 )
	if ( exports.ngadministration:getPlayerStaffLevel ( p, 'int' ) >= 2 ) then
		outputChatBox ( "----- Baraye Staff -----", p, 255, 255, 0 )
		outputChatBox ( "/makeevent - Sakhtane event", p, 255, 255, 0 )
		for i, v in pairs ( events ) do
			outputChatBox ( "        "..tostring ( i ).." -> "..tostring ( v.name ), p, 255, 255, 0 )
		end
		outputChatBox ( "/stopevent - Laghv kardane evente dar hale ejra", p, 255, 255, 0 )
	end
end )
