...
guiGridListAddColumn(Fisherman.GUI.list, "Meghdar", 0.2)
guiGridListAddColumn(Fisherman.GUI.list, "Arzesh", 0.2)
guiGridListSetSortingEnabled ( Fisherman.GUI.list, false )
Fisherman.GUI.close = guiCreateButton(286, 290, 143, 41, "Bastan", false, Fisherman.GUI.window)
...
function fisherman_onSellMarkerHit ( p )
	...
	if ( getElementData ( p, "Job" ) ~= "Fisherman" ) then
		return exports.NGMessages:sendClientMessage ( "Shoma Fisherman nistid", 255, 255, 255 )
	end
	if ( isPedInVehicle ( p ) ) then
		return exports.NGMessages:sendClientMessage ( "Baraye doroshe kala haye khod nabayad dar vasileye naghlie bashid", 255, 255, 255 )
	end
	
	if ( fisherman_getClientCatchTotal ( ) == 0 ) then
		return exports.NGMessages:sendClientMessage ( "Shoma hich kalai baraye forosh nadarid", 255, 255, 255 )
	end
...
addEventHandler ( "onClientPlayerVehicleEnter", root, function ( v, s )
	if ( source == localPlayer and getElementData ( source, "Job" ) == "Fisherman" and not fisherman_isRenderTextActive and getElementModel( v ) == 453 and s == 0 ) then
		...
		exports.NGMessages:sendClientMessage ( "Baraye seyde mahi, dore daryache ranandegi konid", 0, 255, 255 )
...
function fisherman_onClientCatchRender ( )
	...
	if ( fisherman_nextCatchTick and getTickCount ( ) >= fisherman_nextCatchTick ) then
		...
		if ( fisherman_getItemCountInNet ( ) == fisherman_maxCatch ) then
			return exports.NGMessages:sendClientMessage ( "Sabade seyde shoma por ast, be ghesmate forosh beravid va kala haye khod ra befroshid", 0, 255, 0 )
		end
		local item = fisherman_getRandomItem ( )
		exports.NGMessages:sendClientMessage ( "Shoma yek "..item.." be arzeshe $"..tostring(fisherman_itemsToCatch[item]).." seyd kardid!", 255, 255, 0 )
...
addCommandHandler ( "net", function ( )
	...
	if ( guiGetVisible ( Fisherman.GUI.window ) ) then
		...
		guiGridListSetItemText ( Fisherman.GUI.list, r, 3, "", true, true )
		guiGridListSetItemText ( Fisherman.GUI.list, r, 2, "", true, true )
		guiGridListSetItemText ( Fisherman.GUI.list, r, 1, "Majmoo: $"..tostring(totalPrice), true, true )
	end
end )
...
Fisherman.GUI2.close = guiCreateButton(353, 295, 108, 25, "Bastan", false, Fisherman.GUI2.window)
...
Fisherman.GUI2.caughtFish = guiCreateLabel(10, 118, 437, 18, "Mahiye seyd shode: N/A", false, Fisherman.GUI2.window)
...
addEventHandler ( "NGJobs:Fisherman:OnServerSendClientJobInformationForInterface", root, function ( data )
	...
	guiSetText ( Fisherman.GUI2.caughtFish, "Mahiye seyd shode: ".. tostring ( data.caughtFish ) )
