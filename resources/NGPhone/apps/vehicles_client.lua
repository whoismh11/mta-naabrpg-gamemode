...
addEventHandler ( "NGPhone:App->Vehicles:onClientGetVehicles", root, function  ( cList ) 
	...
	guiGridListClear ( pages['vehicles'].grid )
	if ( #cList == 0 ) then
		guiGridListSetItemText ( pages['vehicles'].grid, guiGridListAddRow ( pages['vehicles'].grid ), 2, "Shoma hich vasileye naghliei nadarid.", true, true )
	else
		...
		if ( impoundedVehicles ~= 0 ) then
			exports['NGMessages']:sendClientMessage ( "Shoma "..impoundedVehicles.." vasileye naghlieye toghif shode darid.", 255, 255, 0 )
		end
	end
end )
