...
addEventHandler ( "NGAmmunation:Module->Training:onClientHitMarker", root, function ( s )
	...
	if not ( d[allowedWeapons[w]] ) then 
		return exports.NGMessages:sendClientMessage ( "In aslahe mojaz be estefade baraye tamrin nemibashad.", 255, 255, 0 )
	end
	if ( d[allowedWeapons[w]] > 90 ) then
		return exports.NGMessages:sendClientMessage ( "In aslahe hade aksar karayi ra darad!", 255, 255, 0 )
	end
...
function Training_OnRender ( )
	...
	dxDrawText ( "Dar markaze amoozeshe aslahe shoma mitavanid maharat haye tirandazi khod ra afzayesh dahid. Bishtare aslahe ha (dar F3 -> Stats) daraye darajeye 0 ta 100 hastand. Dar inja shoma mitavanid darajeye aslahe ra afzayesh dahid ta betavanid ba anha saritar shelik konid, shoma mitavanid duel va ... dashte bashid.", windowX + ( rsx * 20 ), windowY + ( rsy * 40 ), ( windowX+windowW)-(rsx * 20 ), (windowY+windowH)-(rsy*50), tocolor ( 255, 255, 255, 255 ), rsy*1.1, "default-bold", "left", "top", false, true )
	dxDrawText ( "Aslahe entekhab shode: N/A\nMaharat: 0/100\nHazineye har jalase: $1,200", windowX + ( rsx * 20 ), windowY + ( rsy * 225 ), ( windowX+windowW)-(rsx * 20 ), (windowY+windowH)-(rsy*280), tocolor ( 255, 200, 65, 255 ), rsy*1.1, "default-bold", "left", "top", false, true )
	dxDrawRectangle ( windowX+(rsx*20), info.rect.y, windowW-(rsx*40), rsy*35, tocolor ( 0, 0, 0, 150 ) )
	
	dxDrawText ( "Taiid va Tamrin",  info.accept.x, info.accept.y, windowX+(windowW-(rsx*40)), info.accept.y+(rsy*35), tocolor(255,255,255,255), rsy*1.2, "default-bold", "center", "center" )
	dxDrawText ( "Khorooj",  info.exit.x, info.exit.y, windowX+(windowW-(rsx*40)), info.exit.y+(rsy*35), tocolor(255,255,255,255), rsy*1.2, "default-bold", "center", "center" )
	...
	if ( k == "space" ) then
		...
		elseif ( selectedOption == 0 ) then
			...
			if ( not allowedWeapons [ getPedWeapon ( localPlayer ) ] ) then
				return exports.NGMessages:sendClientMessage ( "In aslahe mojaz nemibashad!" )
			end
			
			if ( getPlayerMoney ( ) < 1200 ) then
				return exports.NGMessages:sendClientMessage ( "Shoma $1,200 baraye tamrin ba in aslahe ra nadarid!", 255, 255, 0 )
			end
...
function Training_UserStats ( )
	...
	dxDrawText ( "Amar", winX, winY+(rsy*5), winX+winW, winY+winH, tocolor ( 255, 255, 255, 255 ), rsy*2, "default-bold", "center" )
	dxDrawText ( "Tedade koshte shode: "..tostring(training.TargetShot), winX+(rsx*25), winY+(rsy*50), 0, 0, tocolor ( 255, 160, 0, 255 ), rsy*1.5, "default-bold" )
	dxDrawText ( "Tedade morede niyaz: "..tostring(training.RequiredPeds), winX+(rsx*25), winY+(rsy*90), 0, 0, tocolor ( 255, 160, 0, 255 ), rsy*1.5, "default-bold" )
		...
		if ( training.TargetShot == training.RequiredPeds ) then
			...
			setTimer ( function ( )
				...
				if ( trainingWeapon ~= getPedWeapon ( localPlayer ) ) then
					return exports.NGMessages:sendClientMessage ( "Shoma aslaheye khod ra taghir dadid.... Hich maharati ezafe nashod!", 255, 0, 0 )
				end
				...
				exports.NGMessages:sendClientMessage ( "Aslahe level up shod! Levele jadid: "..d[allowedWeapons[w]].."/100", 0, 255, 0 )
...
function Training_CountdownRender ( )
	...
	if ( t == 0 ) then
		t = "Be samte ahdaf tirandazi konid."
		if ( h > ( rsy * 75 ) ) then
			h = h - ( rsy * 10 )
