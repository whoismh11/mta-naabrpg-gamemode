...
function user_shop.functions:createMainGui ( )
	...
	guiGridListAddColumn(user_shop.gui.main.list, "Gheymat/1", 0.2)
	guiGridListAddColumn(user_shop.gui.main.list, "Meghdar", 0.2)
	guiGridListAddColumn(user_shop.gui.main.list, "Foroshande", 0.25)
	guiGridListSetItemText ( user_shop.gui.main.list, guiGridListAddRow ( user_shop.gui.main.list ), 1, "Dar hale download list... Lotfan sabr konid", true, true )
	...
	user_shop.gui.main.purchase = guiCreateButton(49, 388, 109, 27, "Kharid ha", false, user_shop.gui.main.window)
	...
	user_shop.gui.main.exit = guiCreateButton(558, 388, 90, 27, "Khoroj", false, user_shop.gui.main.window)
	user_shop.gui.main.myitems = guiCreateButton(460, 388, 90, 27, "Item haye man", false, user_shop.gui.main.window)
...
_G['user_shop.functions:purchaseCurrentItem'] = function ( )
	...
	if ( row == -1 ) then
		return exports.NGMessages:sendClientMessage ( "Hich itemi ra entekhab nakardeid.", 255, 255, 0 )
	end
	
	local data = guiGridListGetItemData ( user_shop.gui.main.list, row, 1 )
	local ammount = guiGetText ( user_shop.gui.main.purchAmount )
	if ( ammount == "" ) then
		return exports.NGMessages:sendClientMessage ( "Lotfan meghdari ke mikhahid kharidari konid ra vared konid", 255, 255, 0 )
	end
	
	local ammount = tonumber ( ammount )
	if ( ammount < 0 ) then
		return exports.NGMessages:sendClientMessage ( "meghdar sahih nist", 255, 0, 0 )
	end
	
	if ( ammount > data.quantity ) then
		return exports.NGMessages:sendClientMessage ( "Foroshande faghat "..tostring(data.quantity).." ta baraye forosh darad.", 255, 255, 0 )
	end
 ...
 addEventHandler ( "NGShops:Module->UserShop:onClientReciveList", root, function ( list )
	...
	if ( type ( list ) == "table" and table.len ( list ) > 0 ) then
		...
		guiGridListSetItemText ( user_shop.gui.main.list, guiGridListAddRow ( user_shop.gui.main.list ), 1, "Hich itemi vojod nadarad", true, true )
	end
 end )
...
function user_shop.functions:createProfileGui ( )
	...
	user_shop.gui.pro.add_lbl = guiCreateLabel(sx*10, sy*23, sx*260, sy*16, "Ezafe kardane item", false, user_shop.gui.pro.window)
	...
	guiGridListAddColumn(user_shop.gui.pro.add_items, "Meghdar", 0.3)
	user_shop.gui.pro.add_lbl2 = guiCreateLabel(sx*285, sy*49, sx*164, sy*17, "Gheymate har kodam:", false, user_shop.gui.pro.window)
	user_shop.gui.pro.add_price = guiCreateEdit(sx*285, sy*66, sx*164, sy*24, "", false, user_shop.gui.pro.window)
	user_shop.gui.pro.add_lbl3 = guiCreateLabel(sx*286, sy*100, sx*164, sy*17, "Meghdar:", false, user_shop.gui.pro.window)
	user_shop.gui.pro.add_quan = guiCreateEdit(sx*285, sy*117, sx*164, sy*24, "", false, user_shop.gui.pro.window)
	user_shop.gui.pro.add_add = guiCreateButton(sx*337, sy*156, sx*112, sy*33, "Ezafe kardan be shop", false, user_shop.gui.pro.window)
	user_shop.gui.pro.my_lbl = guiCreateLabel(sx*10, sy*235, sx*294, sy*18, "Item haye man", false, user_shop.gui.pro.window)
	...
	guiGridListAddColumn(user_shop.gui.pro.my_grid, "Meghdar", 0.2)
	guiGridListAddColumn(user_shop.gui.pro.my_grid, "Gheymat", 0.2)
	user_shop.gui.pro.exit = guiCreateButton(346, 419, 112, 33, "Bazgasht be shop", false, user_shop.gui.pro.window)
	addEventHandler ( "onClientGUIClick", root, _G['user_shop.functions:onClientGUIClick'] )
	addEventHandler ( "onClientGUIChanged", root, _G['user_shop.functions:onClientGUIChanged'] )
	...
	guiGridListSetItemText ( user_shop.gui.pro.my_grid, guiGridListAddRow ( user_shop.gui.pro.my_grid ), 1, "Darhale load kardane item ha...", true, true )
	showCursor ( true )
	...
	if ( not items or type ( items ) ~= "table" or table.len ( items ) <= 0 ) then
		return guiGridListSetItemText ( user_shop.gui.pro.add_items, guiGridListAddRow ( user_shop.gui.pro.add_items ), 1, "Itemi vojod nadarad", true, true )
	else
...
function user_shop.functions:attemptToSellMyItem ( )
	...
	if ( r == -1 )  then
		return exports.NGMessages:sendClientMessage ( "Hich itemi entekhab nashode", 255, 255, 0 )
	end
	
	local price = guiGetText ( user_shop.gui.pro.add_price )
	if ( price == "" ) then return exports.NGMessages:sendClientMessage ( "Gheymat ra vared konid", 255, 255, 0 ) end
	local price = tonumber ( price )
	
	local quan = guiGetText ( user_shop.gui.pro.add_quan )
	if ( quan == "" ) then return exports.NGMessages:sendClientMessage ( "Meghdar ra vared konid", 255, 255, 0 ) end
	...
	if ( price > itemPrices[item:lower()].max or price < ( itemPrices[item:lower()].min or 0 ) ) then
		return exports.NGMessages:sendClientMessage ( "Bishtarin gheymat baraye in item $"..tostring(itemPrices[item:lower()].max).." mibashad va kamtarin gheymat ham $"..tostring(itemPrices[item:lower()].min), 255, 255, 0 )
	elseif ( quan == 0 ) then
		return exports.NGMessages:sendClientMessage ( "Meghdar eshtebah ast", 255, 0, 0 )
	elseif ( quan > quantity ) then
		return exports.NGMessages:sendClientMessage ( "Shoma in meghdar az in item nadarid.", 255, 255, 0 )
	end
...
addEventHandler ( "NGShops:Module->UserShop:sendClientItems", root, function ( list )
	...
	if ( table.len ( list ) == 0 ) then
		guiGridListSetItemText ( user_shop.gui.pro.my_grid, guiGridListAddRow ( user_shop.gui.pro.my_grid ), 1, "Shoma hich itemi dar shop nadarid", true, true )
		return
	end
	for i, v in pairs ( list ) do
		...
		guiGridListSetItemText ( user_shop.gui.pro.my_grid, r, 3, "$"..tostring ( v.amountperone ).."/Vahed", false, false )
...
_G['user_shop.functions:loadMarkers'] = function ( )
	...
	for i,v in pairs ( user_shop.locs ) do
		...
		if ( makeBlip and user_shop.blips ) then
			user_shop.blips[i] = createBlip ( x, y, z, 36, 2, 255, 255, 255, 255, 0, 450 )
		end
	end
end
