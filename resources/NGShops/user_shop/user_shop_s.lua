...
addEventHandler ( "NGShops:Module->UserShop:onClientAttemptBuyItem", root, function ( id, amnt )
	...
	if ( getPlayerMoney ( source ) < totalPrice ) then
		return exports.NGMessages:sendClientMessage ( "Shoma $"..totalPrice.." baraye kharide "..amnt.." adad az in item niaz darid. Shoma az pasesh bar nemiaid.", source, 255, 255, 0 )
	end
	...
	exports.NGMessages:sendClientMessage ( "Shoma "..amnt.." adad "..itemList[d.item:lower()].." be gheymate $"..totalPrice.." az "..d.seller_account.." kharidari kardid!", source, 0, 255, 0 )
	...
	if ( d.seller_account:lower ( ) ~= "console" ) then
		...
		for i, v in pairs ( getElementsByType ( "player" ) ) do
			...
			if ( not isGuestAccount ( a ) and getAccountName ( a ) == d.seller_account ) then
				...
				exports.NGMessages:sendClientMessage ( getPlayerName ( source ).." meghdare "..amnt.." adad "..itemList[d.item:lower()].." kharidari kard, be gheymate $"..totalPrice, v, 0, 255, 0 )
				givenMoney = true
				break
			end
		end
...
addEventHandler ( "NGShops:Module->UserShop:addItemToShop", root, function ( item, quantity, price )
	...
	if ( isTimer ( user_shop.spamTimers [ acc ] ) ) then
		return exports.NGMessages:sendClientMessage ( "Shoma har 5 daghighe mitavanid item be shop ezafe konid ("..calculateEngTime ( getTimerDetails ( user_shop.spamTimers [ acc ] ) ).." dige)", source, 255, 0, 0 )
	end
	...
	exports.NGMessages:sendClientMessage ( "Shoma "..quantity.." adad "..tostring(itemList[item:lower()]).." iteam be gheymate $"..tostring(price).." babate har kodam be shop ezafe kardid", source, 0, 255, 0 )
end )
