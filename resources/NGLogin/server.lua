	...
	if ( user and pass and type ( user ) == 'string' and type ( pass ) == 'string' ) then
		...
		if ( account ) then
			if ( not logIn ( source, account, pass ) ) then
				message ( source, "Password eshtebah ast." )
				return false
			end
			...
			message ( source, "Accounte nashenas." )
			return false
		end
	end
	return false
...
function attemptRegister ( user, pass )
	if ( user and pass and type ( user ) == 'string' and type ( pass ) == 'string' ) then
		...
		if ( not account ) then
			...
			if ( account ) then
				if ( not logIn ( source, account, pass ) ) then
					return message ( source, "Vorod na movafagh bood." )
				end
				exports['NGLogs']:outputActionLog ( getPlayerName ( source ).." dar server sabtenam kard" )
				...
				message ( source, "Ezafe kardane account namovafagh bood.\nLotfan be admin gozaresh dahid." )
			end
		else
			message ( source, "In account mojod mibashad." )
		end
	end
	return false
end
