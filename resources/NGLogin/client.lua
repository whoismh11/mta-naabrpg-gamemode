...
function dxDrawLoginPanel( )
	...
	if ( isOpen ) then
		...
		dxDrawText("Vorod", pX, pY, pX+485, pY+48, tocolor(255, 255, 255, alpha), 1.50, "bankgothic", "center", "center", false, false, false, false, false)
		...
		dxDrawText("Vorod", pX+166, pY+167, 600, 468, tocolor( 0, 0, 0, alpha), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
		dxDrawText("Vorod", pX+166, pY+167, 600, 468, tocolor( r, g, b, alpha), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
		local r, g, b = unpack ( data.reg.colors )
		dxDrawText("Sabtenam", pX+305, pY+167, 794, 468, tocolor(0, 0, 0, alpha), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
		dxDrawText("Sabtenam", pX+305, pY+167, 794, 468, tocolor(r, g, b, alpha), 1.00, "bankgothic", "left", "top", false, false, false, false, false)
		
		local r, g, b = unpack ( data.remember.colors )
		dxDrawText("Vorod ra be khater bespar", pX+30, pY+177, 794, 468, tocolor(0, 0, 0, alpha), 1.00, "default-bold", "left", "top", false, false, false, false, false)
		dxDrawText("Vorod ra be khater bespar", pX+30, pY+177, 794, 468, tocolor(r, g, b, alpha), 1.00, "default-bold", "left", "top", false, false, false, false, false)
...
function closeLogin ( user, _, intro, ip )
	...
	if ( sx < 1000 or sy < 700 ) then
		for i=1, 3 do 
			outputChatBox ( "Shoma ba resolution paiin bazi mikonid, Bazi az GUI element ha be dorosti be nazar naresad. Lotfan ba resolution balatar bazi konid", 255, 0, 0 )
		end
	end
end
			...
			if ( x >= pX+156 and x <= pX+246 ) then
				if ( string.gsub ( user, ' ', '' ) ~= '' and string.gsub ( pass, ' ', '' ) ~= '' ) then
					if ( string.find ( user, '%s' ) ) then
						return sendError ( "Space (' ') Mojaz nist\nbaraye Username." )
					...
					sendError ( "Username va Password baraye vorod be server\nelzami mibashad." )
				end
			...
			elseif ( x >= pX+300 and x <= pX+440 ) then
				if ( string.gsub ( user, ' ', '' ) ~= '' and string.gsub ( pass, ' ', '' ) ~= '' ) then
					if ( string.find ( user, '%s' ) ) then
						...
						return sendError ( "Space mojaz nist\nbaraye password." )
					end if ( string.len ( user ) < 3 ) then
						return sendError ( "Username bayad hade aghal\n3 character bashad." )
					end if ( string.len ( pass ) < 6 ) then
						return sendError ( "Password bayad hade aghalt\n6 character bashad." )
					end
