...
function cruiseControlData()
	if (not cruiseEnabled) then
		...
		if ( speed < 0.1 ) then
			return exports.NGMessages:sendClientMessage ( "Shoma baraye faal kardane cruise besiar aheste miranid", 255, 255, 0 )
		end
		...
		exports["NGMessages"]:sendClientMessage("Cruise Control faal shod.",0,255,0)
	else
		cruiseEnabled = false
		removeEventHandler("onClientRender",getRootElement(),cruiseControl)
		unbindKey("s","down",cruiseControlOff)
		...
		unbindKey("num_sub","down",decreaseCruiseSpeed)
		setPedControlState("accelerate",false)
		exports["NGMessages"]:sendClientMessage("Cruise Control gheyre faal shod.",0,255,0)
	end
end
