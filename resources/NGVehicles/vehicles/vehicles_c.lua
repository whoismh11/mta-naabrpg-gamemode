...
option['show'] = guiCreateButton((rSX*295), (rSY*28), (rSX*162), (rSY*40), "Namayesh", false, Vehicles)
option['recover'] = guiCreateButton((rSX*295), (rSY*28)+((rSY*40))+(rSY*5), (rSX*162), (rSY*40), "Bazyabi", false, Vehicles)
option['sell'] = guiCreateButton((rSX*295), (rSY*28)+((rSY*40)*2)+(rSY*10), (rSX*162), (rSY*40), "Forosh", false, Vehicles)
option['give'] = guiCreateButton((rSX*295), (rSY*28)+((rSY*40)*3)+(rSY*15), (rSX*162), (rSY*40), "Dadan", false, Vehicles)
option['close'] = guiCreateButton((rSX*295), (rSY*28)+((rSY*40)*4)+(rSY*20), (rSX*162), (rSY*40), "Bastan", false, Vehicles)  
guiSetVisible ( Vehicles, false )
guiWindowSetSizable(Vehicles, false)
guiGridListAddColumn(cars, "ID", 0.15)
guiGridListAddColumn(cars, "Vasileye naghlie", 0.75)
guiGridListSetSortingEnabled ( cars, false )

local GiveWindow = guiCreateWindow( ( sx / 2 - 449 / 2 ), ( sy / 2 - 401 / 2 ), 449, 401, "Dadane vasileye naghlie", false)
...
guiGridListAddColumn(gridGive, "Esm", 0.9)
btnGiveGive = guiCreateButton(302, 354, 137, 33, "Dadan", false, GiveWindow)
btnGiveCancel = guiCreateButton(151, 354, 137, 33, "Laghv", false, GiveWindow)   
...
addEventHandler ( "NGVehicles:onServerSendClientVehicleList", root, function  ( cList ) 
	...
	guiGridListClear ( cars )
	if ( #cList == 0 ) then
		guiGridListSetItemText ( cars, guiGridListAddRow ( cars ), 2, "Shoma vasileye naghliei nadarid.", true, true )
	else
		...
		if ( impoundedVehicles ~= "" ) then
			exports['NGMessages']:sendClientMessage ( "Vasile haye naghlie: "..impoundedVehicles.." Toghif shodand.", 255, 255, 0 )
		end
	end
end )
...
function buttonClicking ( )
	...
	elseif ( source == cars ) then
		...
		if ( row ~= -1 ) then
			...
			if ( visible ) then
				guiSetText ( option['show'], "Penhan" )
				vehicleData[index][9] = 1
			else
				guiSetText ( option['show'], "Namayesh" )
	...
	elseif ( source == option['sell'] ) then
		...
		if ( row ~= -1 ) then
			...
			if ( visible == 1 ) then visible = true else visible = false end
			if ( visible ) then
				return exports['NGMessages']:sendClientMessage ( "Vasileye naghlieye khod ra makhfi konid ghabl az forosh.", 255, 255, 0 )
			end
	...
	elseif ( source == option['give'] ) then
		...
		if ( row ~= -1 ) then
			...
			if ( visible == 1 ) then visible = true else visible = false end
			if ( visible ) then
				return exports['NGMessages']:sendClientMessage ( "Lotfan vasileye naghlie ra makhfi konid baraye dadan.", 255, 0, 0 )
			end
			...
			if ( vehID ) then
				...
				if ( count == 0 ) then
					guiGridListSetItemText ( gridGive, guiGridListAddRow ( gridGive ), 1, "Moteasefim, dar hale hazer player online nemibashad.", true, true  )
				end
			end
		end
	...
	elseif ( source == btnGiveGive ) then 
		...
		if ( row ~= -1 ) then
			...
			if ( not isElement ( getPlayerFromName ( pName ) ) ) then return exports['NGMessages']:sendClientMessage ( "Moteasefim, in player vojod nadarad.", 255, 0, 0 ) end
			
			if ( vehicleData[givingVehicle][9] == 1 ) then return exports['NGMessages']:sendClientMessage ( "Vasileye naghlie ra makhfi konid ta betavanid aan ra bedahid.", 255, 0, 0 ) end
			...
			exports['NGMessages']:sendClientMessage ( "Playeri ke mikhahin vasileye naghlie khod ra be oo bedahid entekhab konid.", 255, 0, 0 )
		end
	elseif ( source == option['recover'] ) then
		local row, col = guiGridListGetSelectedItem ( cars )
		if ( row == -1 ) then return end
		local data = vehicleData[guiGridListGetItemData ( cars, row, 1 )]
		if ( data[9] == 1 ) then return exports['NGMessages']:sendClientMessage ( "Baraye tamir kardane vasileye naghlieye khod ebteda aan ra makhfi konid.", 255, 0, 0 ) end
