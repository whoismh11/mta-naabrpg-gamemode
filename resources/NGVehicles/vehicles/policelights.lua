...
bindKey ( 'horn', 'down', function ( )
	...
	if ( car and getVehicleController ( car ) == localPlayer and serviceVehicles[getElementModel(car)] ) then
		...
		if stat then
			exports['NGMessages']:sendClientMessage ( "Cheragh khamosh!", 255, 255, 0 )
			setElementData ( car, 'NGVehicles:lightState', 0 )
		else
			exports['NGMessages']:sendClientMessage ( "Cheragh roshan!", 0, 255, 255 )
		end
	end
end )
...
addEventHandler ( "onClientPlayerVehicleEnter", root, function ( veh, seat )
	if ( source == localPlayer and seat == 0 and serviceVehicles[getElementModel(veh)] ) then
		exports['NGMessages']:sendClientMessage ( "In vasileye naghlie daraye cheragh haye sefareshi ast! azhir ra baraye roshan/khamosh kardane anha estefade konid .", 0, 255, 0 )
	end
end )
