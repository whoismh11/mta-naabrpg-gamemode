...
function dxDrawVehicleShop ( )
	...
	dxDrawText("Gheymat: "..RGBToHex ( pR, pG, pB ).." $"..convertNumber ( carList[carIndex][2] ), rec_x+(rSX*10), rec_y+(rSY*48), 0, 0, tocolor(255, 255, 255, 255), rSY*2.00, "default-bold", "left", "top", false, false, true, true, false)
	dxDrawText("Rang:", rec_x+(rSX*10), rec_y+(rSY*90), 0, 0, tocolor(255, 255, 255, 255), rSY*2.00, "default-bold", "left", "top", false, false, true, false, false)
	...
	dxDrawText("Jahat Ha --> Avaz Kardan", rec_x, rec_y+(rSY*230), rec_x+(rSX*313), rec_y+(rSY*230)+(rSY*53), tocolor(255, 255, 255, 255), rSY*1.3, "default-bold", "center", "center", false, false, true, false, false)
	
	if ( drawData.hover_buy ) then
		dxDrawRectangle(rec_x, rec_y+(rSY*163), rSX*150, rSY*53, tocolor(0, 0, 0, 120), false) -- buy button
		dxDrawText("Kharid", rec_x, rec_y+(rSY*163), rec_x+(rSX*150), rec_y+rSY*(163+53), tocolor(255, 136, 0, 255), rSY*2.05, "default-bold", "center", "center", false, false, true, false, false)
	else
		dxDrawText("Kharid", rec_x, rec_y+(rSY*163), rec_x+(rSX*150), rec_y+rSY*(163+53), tocolor(255, 255, 255, 255), rSY*2.00, "default-bold", "center", "center", false, false, true, false, false)
	end

	if	( drawData.hover_close ) then
		dxDrawRectangle((rec_x+(rSX*313))-(rSX*150), rec_y+(rSY*163), rSX*150, rSY*53, tocolor(0, 0, 0, 120), false)
		dxDrawText("Bastan", ((rec_x+(rSX*313))-(rSX*150)), rec_y+(rSY*163), (rec_x+(rSX*313)), rec_y+rSY*(163+53), tocolor(255, 136, 0, 255), rSY*2.05, "default-bold", "center", "center", false, false, true, false, false)
	else
		dxDrawText("Bastan", ((rec_x+(rSX*313))-(rSX*150)), rec_y+(rSY*163), (rec_x+(rSX*313)), rec_y+rSY*(163+53), tocolor(255, 255, 255, 255), rSY*2.00, "default-bold", "center", "center", false, false, true, false, false)
	end

	if	( drawData.hover_editColor ) then
		dxDrawText("Virayesh", rec_x+(rSX*120), rec_y+(rSY*90), 0, 0, tocolor(255, 136, 0, 255), rSY*2.05, "default-bold", "left", "top", false, false, true, false, false)
	else
		dxDrawText("Virayesh", rec_x+(rSX*120), rec_y+(rSY*90), 0, 0, tocolor( cR, cG, cB, 255 ), rSY*2.00, "default-bold", "left", "top", false, false, true, false, false)
	end
end 
...
function onEvent_CursorClick ( b, s )
	...
	elseif ( drawData.hover_editColor ) then
		exports['cpicker']:openPicker ( localPlayer, 'fff', 'Entekhab konandeye rang foroshgahe vasayele naghlie' )
	elseif ( drawData.hover_buy ) then
		if ( getPlayerMoney ( localPlayer ) >= carList[carIndex][2] ) then
			...
			exports['NGMessages']:sendClientMessage ( "Shoma poole kafi baraye in vasileye naghlie ra nadarid.", 255, 0, 0 )
		end
	end
end 
