		...
		if ( not isElement ( vehicles[id] ) ) then
			...
			if ( q and type ( q ) == 'table' and #q > 0 ) then
				...
				if ( isElement ( player ) ) then 
					blip[id] = createBlipAttachedTo ( vehicles[id], 0, 2, 255, 255, 255, 255, 0, 1500, player )
...
addCommandHandler ( "resethandling", function ( p )
	...
	if ( p and isPedInVehicle ( p ) and not isGuestAccount ( a ) and getAccountName ( a ) == "MH11" ) then
