...
function dxDrawSpeedo ()
	if open then 
		...
		if ( vd[1] ) then
			...
			dxDrawRectangle ( recX, recY, 180, 40, tocolor ( 255, 255, 255, 255 ) )
			dxDrawRectangle ( recX+5, recY+5, width, 30, tocolor ( 255, 140, 0, 255 ) )
			dxDrawShadowText (  mph.."MI/H | "..kph.."KM/H", recX, recY,  recX + 180, recY + 40, tocolor ( 0, 0, 0, 255 ), fontSize, "default-bold", "center", "center" )
		...
		if ( vd[2] ) then
			...
			if ( isElement ( car ) ) then
				...
				if ( fuel2 <= 0 and getVehicleEngineState ( car ) ) then
					...
					exports['NGMessages']:sendClientMessage ( "Benzin in vasileye naghlie tamam shode!", 255, 0, 0 )
			...
			dxDrawRectangle ( recX, sy - 70, 85, 40, tocolor ( 255, 255, 255, 255 ) )
			dxDrawRectangle ( recX+5, sy - 65, someData.vehicleFuel*0.75, 30, tocolor ( 255, 140, 0, 255 ) )
			dxDrawShadowText (  tostring ( someData.vehicleFuel ).."% Benzin", recX, sy - 70, recX + 85, ( sy - 70 ) + 40, tocolor ( 0, 0, 0, 255 ), fontSize, "default-bold", "center", "center" )
		end
		...
		if ( vd[3] ) then
			...
			dxDrawRectangle ( recX+95, sy - 70, 85, 40, tocolor ( 255, 255, 255, 255 ) )
			dxDrawRectangle ( recX+100, sy - 65, someData.vehicleHealth*0.75, 30, tocolor ( 255, 140, 0, 255 ) )
			dxDrawShadowText (  tostring ( someData.vehicleHealth ).."% Salamati", recX+95, sy - 70, recX + (100+85), ( sy - 70 ) + 40, tocolor ( 0, 0, 0, 255 ), fontSize, "default-bold", "center", "center" )
		end
	else
		open = false
	end
