local fuelPrice = math.random ( 1, 5 )
exports.NGMessages:sendClientMessage ( "Gheymate benzin aknoon $"..tostring(fuelPrice).."/gallon mibashad!", root, 255, 255, 0 )
...
setTimer ( function ( )
	local _fuelPrice = math.random ( 1, 5 )
	while ( fuelPrice == _fulePrice ) do
		_fuelPrice = math.random ( 1, 5 )
	end
	...
	for i, v in ipairs ( getElementsByType ( 'vehicle' ) ) do 
		...
		if ( fuel >= 1 and speed > 0 and getVehicleOccupant ( v ) ) then
			...
			if ( warnings[fuel] ) then
				exports['NGMessages']:sendClientMessage ( "Ekhtar, Benzin shoma roo be etmam ast, Akknoon: "..tostring(fuel).."%", getVehicleOccupant ( v ), 255, 0, 0 )
			end
		end
