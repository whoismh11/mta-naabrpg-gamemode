...
function createWaypointLoc ( x, y, z )
	...
	if ( dist <= 20 ) then
		exports.NGMessages:sendClientMessage ( "Shoma faghat "..tostring(math.floor(dist)).." metr az location fasele darid.", 255, 255, 255 )
		return false;
	end
...
function onWaypointRender ( )
	...
	if ( TRACKELEMENT ) then
		if ( not isElement ( TRACKELEMENT ) ) then
			waypointUnlocate ( )
			exports.NGMessages:sendClientMessage ( "Player/chizi ke shoma track kardid digar vojod nadarad.", 255, 0, 0 )
			return
		else
	...
	if ( dist <= 15 ) then
		waypointUnlocate ( )
		exports.NGMessages:sendClientMessage ( "Shoma be maghsad residid!", 0, 255, 0 )
	end
	...
	local t = "Shoma "..tostring(convertNumber(dist)).." metr az maghsad fasele darid"
