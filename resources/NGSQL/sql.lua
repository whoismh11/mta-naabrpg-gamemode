...
VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? );]], 
			...
			'[ [ ] ]', 'None', 'None', 'None', today, autoSerial, autoIP, '0', '0', weapStats, toJSON ( { } ), '16', 'None', nil, toJSON ( { } ) )
	end
	return false
end
...
function savePlayerData ( p, loadMsg, deleteTime )
	if ( p and getElementType ( p ) == 'player' ) then
		if ( not isGuestAccount ( getPlayerAccount ( p ) ) ) then
			...
			local unemloyedSkin = tostring ( getElementData ( p, "NGUser.UnemployedSkin" ) ) or 16
	...
	if ( data and type ( data ) == 'table' ) then
		for i, v in ipairs  ( data ) do
			if ( v['Username'] == acc ) then
				...
				local x = 			tonumber ( v['x'] ) 				or 1546.58
				local y = 			tonumber ( v['y'] ) 				or -1675.31
				local z = 			tonumber ( v['z'] ) 				or 13.56
				local skin = 		tonumber ( v['Skin'] ) 				or 16
				...
				local unemployedSkin=tonumber( v['unemployedskin'] )	or 16
