...
addEventHandler ( "onPlayerLogin", root, function ( )
	if ( not exports['NGAdministration']:isPlayerStaff ( source ) ) then
		sendClientMessage ( getPlayerName ( source ).." varede server shod!", root, 0, 255, 0 )
	else
		local lvl = exports['NGAdministration']:getPlayerStaffLevel ( source, 'string' )
		outputChatBox ( "* "..getPlayerName ( source ).." vared shod - "..lvl, root, math.random ( 100, 250 ), math.random ( 100, 250 ), math.random ( 100, 250 ) )
	end
end )
